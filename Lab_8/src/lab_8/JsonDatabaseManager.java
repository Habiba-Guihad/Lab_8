...
public class JsonDatabaseManager {
    ...
    public void saveUsers() {
        ...
        for (User u : users) {
            ...
            if (u instanceof Student s) {
                ...
                // ProgressData (detailed): completed lessons + quiz attempts
                JSONObject pdObj = studentProgressToJson(s.getProgressData());
                obj.put("progressData", pdObj);
            }

            if (u instanceof Instructor ins) {
                JSONArray createdArr = new JSONArray(ins.getCreatedCourses());
                obj.put("createdCourses", createdArr);
            }
    ...
    // Helper: serialize StudentProgress
    private JSONObject studentProgressToJson(StudentProgress progressData) {
        JSONObject json = new JSONObject();

        // completedLessons map
        JSONObject lessonsJson = new JSONObject();
        // progressData.getCompletedLessons() not present in your class as a named method,
        // but StudentProgress has getCompletedLessons(int) and getProgress map; we'll use getProgress()
        ...
        for (QuizAttempt attempt : attempts) {
            ...
            // optionally include attempt time if available:
            // a.put("attemptTime", attempt.getAttemptTime().toString());
            attemptsArr.put(a);
        }
    ...
    // Helper: load StudentProgress into existing Student
    private void loadStudentProgress(Student student, JSONObject json) {
        ...
        if (attemptsArr != null) {
            for (int i = 0; i < attemptsArr.length(); i++) {
                ...
                // create a minimal Quiz object with title only (other fields unknown)
